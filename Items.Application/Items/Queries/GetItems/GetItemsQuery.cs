...
public class GetItemsQueryHandler : IRequestHandler<GetItemsQuery, IResponse<PaginatedList<ItemDto>>>
{
	...
	public async Task<IResponse<PaginatedList<ItemDto>>> Handle(GetItemsQuery request, CancellationToken cancellationToken)
	{
		...
		if (existingItems is null)
			return new ErrorResponse<PaginatedList<ItemDto>>((int)ErrorCodes.DatabaseGet, ErrorCodes.DatabaseGet.ToString(), Errors.DatabaseGet.Item);
		...
		switch (request.SortBy)
		{
			...
			case GetItemsSortBy.Producer:
				if (request.SortDescending)
					items = items.OrderByDescending(x => x.Producer).ToList();
				else
					items = items.OrderBy(x => x.Producer).ToList();
				break;
			case GetItemsSortBy.Supplier:
				if (request.SortDescending)
					items = items.OrderByDescending(x => x.Supplier).ToList();
				else
					items = items.OrderBy(x => x.Supplier).ToList();
				break;
