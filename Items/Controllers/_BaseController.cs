...
public class BaseController : Controller
{
	...
	protected IActionResult ConvertToResponse<T>(IResponse<T> response)
	{
		...
		return BadRequest(new ErrorApiResponse()
		{
			...
			ErrorCodes = new List<ErrorCode>() { new ErrorCode() { Code = Enum.GetName(typeof(ErrorCodes), response.ErrorCode ?? 0) ?? string.Empty, Description = response.Message ?? string.Empty } }
		});
	}
}
